...
<!-- ####################################################################################################### -->
  <%
  String fconcert = (String)request.getSession().getAttribute("fconcert");
  User user = (User)request.getSession().getAttribute("userdetails");
  System.out.print("Inside jsp"+fconcert);
  System.out.print("Inside jsp"+user.getUid());
...
<div class="wrapper col4">
  <div id="featured_intro">
    ...
    <div class="fl_right">
  	...
  	<font color=black>
  	<b>
  	<p>Concert ID : <%=rs.getString("concert_id") %></p>
   <p>Artist ID : <%=rs.getString("artist_id") %></p>
  <p>Concert Time : <%=rs.getString("concert_time") %></p>
  <p>Venue : <%=rs.getString("venue_name")+","+rs.getString("venue_city") %></p>
  <p>Price : <%=rs.getString("TicketRange") %></p>
  <p>Availability : <%=rs.getString("availablity") %></p>
  <p>Link : <%=rs.getString("ticket_book_link") %></p>
  </b>
  </font>
  ...
  <input type="submit" align='center' name="followunfollow" value="ATTEND"></input>
  <input type = "hidden" name="getcuid" value=<%=fconcert %>></input>
  <input type = "hidden" name="getuid" value="<%=user.getUid()%>"></input>
  <input type = "hidden" name="action" value="attendc"></input>
	</form>
  </center>
