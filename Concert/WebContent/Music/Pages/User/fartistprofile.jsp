...
<!-- ####################################################################################################### -->
  <%
  String aredirect = (String)request.getSession().getAttribute("aredirection");
  User user = (User)request.getSession().getAttribute("userdetails");
  System.out.print("Inside jsp"+aredirect);
  System.out.print("Inside jsp"+user.getUid());
 Connection connect = DBConnection.createConnection();
 
 PreparedStatement ps = connect.prepareStatement("select * from artist where artist_id=?;");
 ps.setString(1,aredirect);
...
<div class="wrapper col4">
  <div id="featured_intro">
    ...
    <div class="fl_right">
  	...
  	<font color=black>
  	<b>
   <p>Artist ID : <%=rs.getString(1)%></p>
  <p> Artist Name : <%=rs.getString(3)%></p>
  <p> Profile : <%=rs.getString(4) %></p>
  <p>Artist Link : <%=rs.getString(5)%></p>
  </b>
  </font>
  ...
  <input type="submit" align='center' name="followunfollow" value="UNFOLLOW"></input>
  <input type = "hidden" name="getauid" value="<%=aredirect%>"></input>
  <input type = "hidden" name="getuid" value="<%=user.getUid()%>"></input>
  <input type = "hidden" name="action" value="unfollowartist"></input>
	</form>
  </center>
