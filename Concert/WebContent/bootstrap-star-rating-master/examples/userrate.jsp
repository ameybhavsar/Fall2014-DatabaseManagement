...
<link href="http://netdna.bootstrapcdn.com/bootstrap/3.1.0/css/bootstrap.min.css" rel="stylesheet">
    ...
    <body>
     <%
	//ArrayList<User> customerlist = (ArrayList)request.getSession().getAttribute("userdetails");
...
<form id="ratestar" name="ratestarform" method="post" action="<%=request.getContextPath() %>/MusicController" >
    ...
    <input id="input-21c" name="userrate" value="0" type="number" class="rating" min=0 max=5 step=1 data-size="sm" >
       	<input type="submit" value="Submit" name="submit">  
       			<input type="hidden" name="usid" value="<%=user.getUid()%>"></input>
 ...
 <script>
  ...
  PreparedStatement ps = connect.prepareStatement("select avg(rating) as avgrate from user_AttendsorPlans where concert_id=? ");
 ps.setString(1, "c103");
...
%>
    <br>Average Rating:<br> 
      <input id="input-21d" value="<%=rs.getInt("avgrate") %>" type="number" class="rating" min=0 max=5 step=0.5 data-size="sm" readonly>
      <%
}
%>
 </html>       
