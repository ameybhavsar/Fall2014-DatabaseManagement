...
<div class="wrapper col4">
 ...
 <h2>
 <center>Artists who meet your search Criteria</center></h2>
      <table summary="Summary Here" cellpadding="0" cellspacing="0">
	   ...
	   ArrayList<Artist> artistlist = (ArrayList)request.getSession().getAttribute("alist");
 if(artistlist!=null){
	 Iterator itr = artistlist.iterator();
	 while(itr.hasNext())
	 {
		 Artist artist = (Artist)itr.next();
%>
          <tr class="light">
            <td><input type = "radio" name="artistfan" value="<%=artist.getArtist_id() %>"></input></td>
		<td><a href="https://en.wikipedia.org/wiki/<%=artist.getArtist_name() %>" target="_blank"><%=artist.getArtist_id() %></a></td>
		<td><%=artist.getArtist_name() %></td>
		<td><%= artist.getProfile() %></td>
		<td><%= artist.getcategory() %></td>
		<td><%= artist.getsubcategory() %></td>
		<td><%= artist.getArtist_detailslink() %></td>
</tr>
...
<input type="hidden" name="action" value="artistfollows"></input>
